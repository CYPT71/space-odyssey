 ...
 * - pattern (the RegExp to match)
 * - replace (the replacement string or function to execute)
 */
const parseMap = [
	{
		// <h1> à <h6>
...
function applyReplacement(matchList, replacement, type) {
	for (const i in matchList) {
		if (!Object.prototype.hasOwnProperty.call(matchList, i)) continue;
		...
		replacement = replacement
			.split("$L" + i)
			.join(String(matchList[i]).length);
	}
...
export function parse(string) {
	...
	parseMap.forEach(function (p) {
		...
		output = output.replace(p.pattern, function () {
			return applyReplacement(arguments, p.replace, p.type);
		});
	});
